...
# python grad_dist.py Z_dist.dat
#
# May need to edit nx_voxels, ny_voxels to match size of input field.
#
...
#grad_u, grad_v = np.gradient(Z)  # 2nd arg is the voxel width?
grad_v, grad_u = np.gradient(Z)  # 2nd arg is the voxel width?
#grad_u, grad_v = np.gradient(Z,20,20)  # 2nd arg is the voxel width?
#grad_v, grad_u = np.gradient(Z,20,20)  # 2nd arg is the voxel width?
...
#plt.quiver(X, Y, U, V, scale_units='x', scale=100) #, scale=0.005)
#plt.quiver(X, Y, U, V) #, scale=0.005)
#plt.quiver(X, Y, U, V, scale_units='x', scale=1) #, scale=0.005)
#plt.quiver(X, Y, grad_u, grad_v, scale_units='x', scale=1) #, scale=0.005)
plt.quiver(X, Y, -grad_u, -grad_v, scale_units='x', scale=1) #, scale=0.005)  # flip for what we *want*
# plt.quiver(X, Y, U, V, scale_units='xy', scale=1.e6) #, scale=0.005)
# plt.quiver(X, Y, U, V, scale=1.e-9) #, scale=0.005)
#plt.quiver(X, Y, U, V, scale=1.e-9) #, scale=0.005)
...
if signed_dist_flag:
    ...
    plt.title("gradient and unsigned dist; grid: " + str(nx_voxels) + "x" + str(ny_voxels))
plt.show()
